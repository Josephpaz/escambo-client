...
export function HistoricoTabs() {
    ...
    // Paginação
    const totalItems = trocasArray.length;
    const totalPages = Math.max(Math.ceil(totalItems / ITEMS_PER_PAGE), 1);
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const endIndex = startIndex + ITEMS_PER_PAGE;
    const trocasPaginadas = trocasArray.slice(startIndex, endIndex);
    ...
    useEffect(() => {
        async function fetchTrocas() {
            ...
            try {
                ...
                // Limpa a lista para evitar dados inconsistentes
                if (currentTab === "enviadas") {
                    setTrocasEnviadas([]);
                } else {
                    setTrocasRecebidas([]);
                    ...
                    <TabsContent value={currentTab}>
                        {loading && (
                            <Stack align="center" py={10}>
                                ...
                                <Text mt={2} color={"#4A4F59"}>Carregando histórico...</Text>
                            </Stack>
                        )}
